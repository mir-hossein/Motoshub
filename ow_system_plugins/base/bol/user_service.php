...
final class BOL_UserService
{
    ...
    public function saveLoginCookie( $userId, $expiredTimestamp )
    {
        ...
        $cookie = hash_hmac('md5', time(), $userId);
        OW::getEventManager()->trigger(new OW_Event('iismobilesupport.save.login.cookie',array('cookie'=>$cookie)));
    ...
    public function deleteUser( $userId, $deleteContent = true )
    {
        ...
        OW::getLogger()->writeLog(OW_Log::INFO, 'delete_user', ['actionType'=>OW_Log::DELETE, 'enType'=>'user', 'enId'=>$userId]);
        return true;
    }
    ...
    {
        if(isset($userId)) {
            $user=$this->findByIdWithoutCache($userId);
            return hash('sha256', IISSecurityProvider::getStaticPepper() . $user->salt . $password);
        }else{
            $salt=md5(UTIL_String::getRandomString(8, 5));
            return array('password'=>hash('sha256', IISSecurityProvider::getStaticPepper() . $salt . $password),'salt'=>$salt);
        }
    }
    ...
    public function processSignIn( $identity, $password, $rememberMe = false )
    {
        ...
        if ( $result->isValid() )
        {
            ...
            {
                $day = 7;
                $eventSaveCookie = OW::getEventManager()->trigger(new OW_Event(IISEventManager::ON_BEFORE_AUTOLOGIN_COOKIE_UPDATE, array('userId' => OW::getUser()->getId(), 'cookie' => '', 'day' => $day)));
                ...
                $loginCookie = $this->saveLoginCookie(OW::getUser()->getId(), (time() + 86400 * $day));
                setcookie('ow_login', $loginCookie->getCookie(), (time() + 86400 * $day), '/', null, null, true);
            }
        }
    ...
    public function addTokenForUser( $userId )
    {
        ...
        $token->setToken(IISSecurityProvider::generateUniqueId($userId));
