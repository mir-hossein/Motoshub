...
class BOL_UserDao extends OW_BaseDao
{
    ...
    public function findUserIdListByQuestionValuesQuery( $questionValues, $isAdmin = false, $aditionalParams = array() )
    {
        ...
        if ( $isAdmin === true )
        {
            ...
            $query = "SELECT * FROM(SELECT $distinct `user`.id, `user`.`activityStamp`, `user`.`username`  FROM {$usersTableName} `user` " .
                " WHERE `user`.id NOT IN (" .
                "SELECT userId FROM `" . OW_DB_PREFIX ."iissecurityessentials_question_privacy` WHERE `questionId` IN ( " . substr($questionIdsList, 0, -2) . ") AND" .
            ...
            case BOL_QuestionService::QUESTION_PRESENTATION_MULTICHECKBOX :
                ...
                if ( !empty ( $value ) )
                {
                    if ( is_array($value) )
                    {
                        ...
                        {
                            $whereClause.= " (`" . $prefix . "`.`textValue` LIKE '[".$val.",%' OR `" . $prefix . "`.`textValue` LIKE '%,".$val.",%' OR `" . $prefix . "`.`textValue` LIKE '%,".$val."]') AND ";
                        }
                        $whereClause.=" 1=1";
