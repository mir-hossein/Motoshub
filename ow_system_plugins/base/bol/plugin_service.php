...
// install plugins list
    ...
    {
        OW::getLogger()->writeLog(OW_Log::INFO, 'plugin_install', ['actionType'=>OW_Log::CREATE, 'enType'=>'plugin', 'enId'=>$key]);
        ...
        try {
            ...
            OW::getLogger()->writeLog(OW_Log::INFO, 'plugin_install', ['actionType'=>OW_Log::CREATE, 'enType'=>'plugin', 'enId'=>$key, 'log'=>'plugin installed and activated success']);
            return $pluginDto;
        }catch (Exception $ex){
...
//            OW::getStorage()->chmod($plugin->getPluginFilesDir(), 0777);
    ...
    {
        OW::getLogger()->writeLog(OW_Log::INFO, 'plugin_uninstall', ['actionType'=>OW_Log::DELETE, 'enType'=>'plugin', 'enId'=>$pluginKey]);
        if ( empty($pluginKey) )
        {
    ...
    {
        OW::getLogger()->writeLog(OW_Log::INFO, 'plugin_activate', ['actionType'=>OW_Log::UPDATE, 'enType'=>'plugin', 'enId'=>$key]);
    ...
    {
        OW::getLogger()->writeLog(OW_Log::INFO, 'plugin_deactivate', ['actionType'=>OW_Log::UPDATE, 'enType'=>'plugin', 'enId'=>$key]);
        $pluginDto = $this->pluginDao->findPluginByKey($key);
