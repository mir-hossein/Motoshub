    ...
    public function onBeforeDecoratorRender( BASE_CLASS_PropertyEvent $e )
    {
        switch ( $e->getProperty('decoratorName') )
        {
            case 'avatar_item':
                ...
                //todo : to make config in iiswidgetplus settings controlling label characters ( This part is commented since avatar labels are to be printed completely )
/*                if ( $e->getProperty('fullLabel') === null )
                {
                    $labelCharacterCount=1;
                    if (OW::getConfig()->configExists('iiswidgetplus', 'label_character_count')){
                        $labelCharacterCount=OW::getConfig()->getValue('iiswidgetplus', 'label_character_count');
                    }
            ...
            if ( $user !== null )
            {
                ...
                else
                {
                    ...
                    if ( $questionsEditStamp >= (int) $updateDetailsStamp )
                    {
                        ...
                        {
                            OW::getEventManager()->trigger(new OW_Event('iismobilesupport.exclude.catch.request'));
