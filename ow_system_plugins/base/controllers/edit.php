...
class BASE_CTRL_Edit extends OW_ActionController
{
    ...
    public function index( $params )
    {
        ...
        if (!empty($params['userId']) && $params['userId'] != $viewerId) {
            ...
            OW::getEventManager()->trigger(new OW_Event(IISEventManager::ON_AFTER_CHANGE_PASSWORD_WIDGET_ADDED, array('component' => $this)));
        ...
        if ($displayPhotoUpload == BOL_UserService::CONFIG_JOIN_DISPLAY_AND_SET_REQUIRED_PHOTO_UPLOAD) {
            ...
            $editAvatar->setRequired();
        }

        $editAvatar->setDescription(BOL_QuestionService::getInstance()->getQuestionDescriptionLang($editAvatar->getName()));
        ...
        OW::getEventManager()->trigger(new OW_Event(IISEventManager::ON_BEFORE_JOIN_FORM_RENDER, array('form' => $editForm, 'controller' => $this,'forEditProfile' =>true, 'editUserId' => $editUserId)));
        // process form
        ...
        $userDisapprove = new OW_Event(IISEventManager::ON_BEFORE_USER_DISAPPROVE_AFTER_EDIT_PROFILE, array(
