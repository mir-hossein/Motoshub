...
class BASE_Import extends DATAIMPORTER_CLASS_Import
{
    ...
    public function import( $params )
    {
        ...
        //import configs
        if ( OW::getStorage()->fileExists($sqlFile) )
        {
        ...
        $string = OW::getStorage()->fileGetContent($configFile);
        ...
        {
            if ( OW::getStorage()->fileExists(OW::getPluginManager()->getPlugin('dataimporter')->getRootDir() . 'langs') )
            {
                Updater::getLanguageService()->updatePrefixForPlugin('dataimporter');
            }
            else if ( OW::getStorage()->fileExists(OW::getPluginManager()->getPlugin('dataimporter')->getRootDir() . 'langs.zip') )
            {
        ...
        while ( true )
        {
            ...
            foreach ( $list as $user )
            {
                for ( $size = 1; $size < 4; $size++ )
                {
                    ...
                    $content = OW::getStorage()->fileGetContent($avatarUrl . '/' . $avatarName);
    ...
    private function importTheme( $importDir )
    {
        ...
        foreach ( $this->configs['controlValue'] as $key => $controlValue )
        {
            ...
            if ( preg_match($url_pattern, $controlValue, $matches) )
            {
                ...
                if ( !empty($matches[1]) && OW::getStorage()->fileExists($imgFile) && OW::getStorage()->isFile($imgFile) )
                {
    ...
    private function importMediaPanelFiles()
    {
        ...
        foreach ($list as $dto)/*@var $dto BOL_MediaPanelFile*/
        {
            ...
            $fileContent = OW::getStorage()->fileGetContent($mediaPanelUrl.'/'.$filename);
