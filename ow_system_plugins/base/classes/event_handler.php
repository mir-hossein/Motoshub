...
class BASE_CLASS_EventHandler
{
    ...
    public function genericInit()
    {
        ...
        $eventManager->bind('base.mandatory_user_approve.add', array($this, 'onMandatoryApproveUserAdd'));
    ...
    public function onMandatoryApproveUserAdd( OW_Event $e )
    {
        $params = $e->getParams();
        $userId = $params['userId'];

        $userService = BOL_UserService::getInstance();
        $admin_url = OW::getRouter()->urlForRoute('admin_users_browse', array('list' => 'unapproved'));
        ...
        foreach ( $moderators as $moderator ) {
            ...
            $isAdmin = (BOL_AuthorizationService::getInstance()->isActionAuthorizedForUser($moderator->userId, BOL_AuthorizationService::ADMIN_GROUP_NAME));
            $isBaseAdmin = (BOL_AuthorizationService::getInstance()->isActionAuthorizedForUser($moderator->userId, 'base'));
            if(!$isAdmin && !$isBaseAdmin){
                continue;
            }
            $url = $isAdmin?$admin_url:$profile_url;

            $params = array(
                'pluginKey' => 'base',
                'entityType' => 'user-add-approve',
                'entityId' => $userId,
            ...
            $data = array(
                ...
                'string' => array(
                    'key' => 'base+user_add_approve_notifications',
                    'vars' => array(
    ...
    public function onMandatoryApproveUserEdit( OW_Event $e )
    {
        ...
        foreach ( $moderators as $moderator ) {
            ...
            $data = array(
                'avatar' => $avatars[$userId],
                'string' => array(
                    'key' => 'base+user_edit_approve_notifications',
                    'vars' => array(
                        'userName' => $userService->getDisplayName($userId),
                        'userUrl' => $userService->getUserUrl($userId),
                        'photoUrl' => $url
    ...
    {
        OW::getEventManager()->trigger(new OW_Event(IISEventManager::ON_BEFORE_CHECK_USER_STATUS));
...
//    }
    ...
    public function onPrivacyAddAction( BASE_CLASS_EventCollector $event )
    {
        ...
        $action = array(
            ...
            'description' => $language->text('base', 'privacy_action_view_profile_description'),
        ...
        $action = array(
            ...
            'description' => $language->text('base', 'privacy_action_view_my_presence_on_site_description'),
            'defaultValue' => $defaultValue
        );
    ...
    public function onNotifyActions( BASE_CLASS_EventCollector $e )
    {
        ...
        $e->add(array(
            ...
            'action' => 'admin-user-add',
            'description' => OW::getLanguage()->text('base', 'user_add_approve_notifications_label'),
            'sectionIcon' => 'ow_ic_file',
            'selected' => true
        ));
    ...
    public function onActionToolbarAddUserBlockActionTool( BASE_CLASS_EventCollector $event )
    {
        ...
        $uniqId = IISSecurityProvider::generateUniqueId("block-");
    ...
    public function onActionToolbarAddUserFeatureActionTool( BASE_CLASS_EventCollector $event )
    {
        ...
        $uniqId = IISSecurityProvider::generateUniqueId("feature-");
    ...
    public function onActionToolbarAddAuthActionTool( BASE_CLASS_EventCollector $event )
    {
        ...
        $userId = (int) $params['userId'];
        $uniqId = IISSecurityProvider::generateUniqueId('change-role-');
    ...
    public function onActionToolbarAddSuspendActionTool( BASE_CLASS_EventCollector $event )
    {
        ...
        $uniqId = IISSecurityProvider::generateUniqueId('pat-suspend-');
        $action[BASE_CMP_ProfileActionToolbar::DATA_KEY_LINK_ID] = $uniqId;
        ...
        if(isset($iisSecuritymanagerEvent->getData()['code'])){
            $suspendCode = $iisSecuritymanagerEvent->getData()['code'];
        ...
        if(isset($iisSecuritymanagerEvent->getData()['code'])){
            $unSuspendCode = $iisSecuritymanagerEvent->getData()['code'];
