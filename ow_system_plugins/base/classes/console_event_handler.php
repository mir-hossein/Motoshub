...
class BASE_CLASS_ConsoleEventHandler
{
    ...
    public function collectItems( BASE_CLASS_ConsoleItemCollector $event )
    {
        ...
        else
        {
            ...
            $eventData = OW::getEventManager()->trigger(new OW_Event(IISEventManager::ON_BEFORE_SIGNIN_BUTTON_ADD, array('item' => $item)));
            $signUpText = $language->text('base', 'console_item_sign_up_label');
            if(isset($eventData->getData()['sso_enabled'])){
                $signUpText = $language->text('iissso', 'sign_up_button');
                if($eventData->getData()['show_sign_in_button']) {
                    $event->addItem($eventData->getData()['sign_in_button'], 2);
                }
                if ($eventData->getData()['show_sign_out_button']) {
                ...
                if ($eventData->getData()['show_sign_up_button']) {
