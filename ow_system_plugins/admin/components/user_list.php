...
{    
    ...
    public function __construct( ADMIN_UserListParams $params )
    {
        ...
        if ( OW::getRequest()->isPost() && !empty($_POST['users']) )
        {
            ...
            $iissmsEvent = OW::getEventManager()->trigger(new OW_Event('iissms.activate.user.sms.code',array('postData'=>$_POST)));
            if(isset($iissmsEvent->getData()['success'])){
                OW::getFeedback()->info($language->text('admin', 'user_feedback_profiles_approved'));
            }
        ...
        $first = ( $page - 1 ) * $onPage;
        $iissmsEvent = OW::getEventManager()->trigger(new OW_Event('iissms.get.userlist.needs.activation',array('type' =>$type,'first'=>$first,'count'=>$onPage)));
        if(isset($iissmsEvent->getData()['userCount']) && isset($iissmsEvent->getData()['userList'])){
            $userList = $iissmsEvent->getData()['userList'];
            $userCount = $iissmsEvent->getData()['userCount'];
        }else {
            switch ($type) {
                case 'recent':
