...
class ADMIN_CTRL_Theme extends ADMIN_CTRL_Abstract
{
    ...
    public function css()
    {
        ...
        {
            if(isset($_POST['mobile-add-css'])){
                $dto = $this->themeService->findThemeByKey(OW::getConfig()->getValue('base', 'selectedTheme'));
                $css = isset($_POST['mobile-add-css']) ? trim($_POST['mobile-add-css']) : '';
                $dto->setMobileCustomCss($css);
        ...
        $this->addForm(new AddMobileCssForm());
    }
...
class AddMobileCssForm extends Form
{

    public function __construct()
    {
        parent::__construct('mobile-css');

        $text = new Textarea('mobile-add-css');
        $dto = BOL_ThemeService::getInstance()->findThemeByKey(OW::getConfig()->getValue('base', 'selectedTheme'));
        $text->setValue($dto->getMobileCustomCss());
        $this->addElement($text);

        $submit = new Submit('mobile-submit');
        $submit->setValue(OW::getLanguage()->text('admin', 'theme_css_edit_submit_label'));
        $this->addElement($submit);
