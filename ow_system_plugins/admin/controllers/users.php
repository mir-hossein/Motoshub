...
class ADMIN_CTRL_Users extends ADMIN_CTRL_Abstract
{
    ...
    private function getMenu()
    {
        ...
        $iissmsEvent = OW::getEventManager()->trigger(new OW_Event('iissms.on.get.users.list.menu.in.admin',array('menuItems' =>$menuItems,'order' => $ord)));
        if(isset($iissmsEvent->getData()['menuItems'])){
            $menuItems = $iissmsEvent->getData()['menuItems'];
        }
    ...
    public function index( array $params )
    {
        ...
        $par->setType($type);
        $par->setExtra($extra);
        $iissmsEvent = OW::getEventManager()->trigger(new OW_Event('iissms.add.activate.sms.code.button',array('type' =>$type)));
        if(isset($iissmsEvent->getData()['buttonSMSActivation'])){
            $par->addButton($iissmsEvent->getData()['buttonSMSActivation']);
        }else {
            switch ($type) {
                case 'recent';
                case 'search':
        ...
        if(OW::getPluginManager()->isPluginActive('iissecurityessentials'))
            $this->assign('changePasswordUrl', OW::getRouter()->urlForRoute('iissecurityessentials.admin.currentSection', array('currentSection' => 4)));
    }
