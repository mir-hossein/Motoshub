   ...
   // System functions
    ...
    _queryCallBack = function( r ) {
        ...
        $.each(r.items || {}, function(itemKey, itemData) {
            ...
            if ( itemData['initializeSetting']!==null){
                popupSetting = itemData['initializeSetting'];
            }
        });
        if(popupSetting!==null){
            var concat="[data-key='"+popupSetting+"']";
            $("div").find(`${concat}`).find('a.ow_ic_gear_wheel.dnd-control.dd_edit').click();
        }
    };
    ...
    $.extend(HiddenDelegate.prototype, DefaultDelegate.prototype, {
        actionClick: function( panel, item, action ) {
            ...
            if ( confirm(OW.getLanguageText('mobile', 'are_you_sure')) ) {
                item.remove();
