...
var checkFileInputSizeAndExtension = function(input) {
    ...
    var ext = '';
    if(input.files[0].name.lastIndexOf('.')>0){
        ext = input.files[0].name.substr(input.files[0].name.lastIndexOf('.')+1);
    }
...
jconfirm.defaults = {
    title: '', titleClass: 'warning', type: 'orange', smoothContent: true, 
...
//                $loginCookie = BOL_UserService::getInstance()->findLoginCookieByUserId($id);
                $day = 7;
                $eventSaveCookie = OW::getEventManager()->trigger(new OW_Event(IISEventManager::ON_BEFORE_AUTOLOGIN_COOKIE_UPDATE, array('userId' => $id, 'cookie' => $_COOKIE['ow_login'], 'day' => $day)));
                ...
                BOL_UserService::getInstance()->updateLoginCookie(trim($_COOKIE['ow_login']), (time() + 86400 * $day));
                setcookie('ow_login', trim($_COOKIE['ow_login']), (time() + 86400 * $day), '/', null, false, true);
            }
        }
    }
