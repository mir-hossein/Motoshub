 ...
 * @since 1.0
 */
class IISMENTION_MCLASS_EventHandler
{
    /**
     * Singleton instance.
     *
     * @var IISMENTION_MCLASS_EventHandler
     */
     ...
     *
     * @return IISMENTION_MCLASS_EventHandler
     */
...
//        OW::getEventManager()->bind(IISEventManager::ON_AFTER_NEWSFEED_STATUS_STRING_READ, array($service,'renderString')); //iisnews
        OW::getEventManager()->bind('hashtag.before_render_string', array($service,'renderString')); //groups, event, video, forum
    ...
    public function onBeforeDocumentRender( OW_Event $event )
    {
        //  if (!startsWith(OW::getRouter()->getUri(), "forum/"))
        {
            ...
            OW::getDocument()->addScript( OW::getPluginManager()->getPlugin('iismention')->getStaticJsUrl() . 'iismention-mobile.js' );
        }
    }
}