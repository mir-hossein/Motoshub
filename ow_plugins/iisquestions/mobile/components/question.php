 ...
 * Time: 1:07 PM
 */
class IISQUESTIONS_MCMP_Question extends OW_MobileComponent
{
    ...
    public function __construct($question, $userId, $editMode = false, $questionIdtmp = false, $options = '')
    {
        ...
        $this->setTemplate(OW::getPluginManager()->getPlugin('iisquestions')->getMobileCmpViewDir() . 'question.html');
    }
    ...
    public function onBeforeRender()
    {
        ...
        $this->addComponent('optionList', new IISQUESTIONS_MCMP_OptionList($this->question, $optionList, $this->userId, $this->editMode));
        if (!isset($this->question) && OW::getUser()->isAuthenticated()){
            $this->addComponent('addOption', new IISQUESTIONS_MCMP_AddOption($this->questionIdtmp));
        }
        else if (IISQUESTIONS_BOL_Service::getInstance()->canCurrentUserAddOption($this->question->getId())){
            $this->addComponent('addOption', new IISQUESTIONS_MCMP_AddOption($this->question->getId()));
        }
